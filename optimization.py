...
# Gradient Descent
def gradient_descent_ensemble(X, y, base_learners, learning_rate=0.01, max_iter=1000, init_weights=None):
    """
...
def ensemble_objective_function(weights, X, y, base_learners):
    ...
    y_ensemble = np.dot(np.column_stack([bl.predict(X) for bl in base_learners]), weights)
...
def optimize_weights_with_de(X, y, base_learners, bounds, max_iter=1000, seed=None):
    ...
    objective_function = lambda weights: ensemble_objective_function(weights, X, y, base_learners)
...
def tune_params(X, y, estimator, params):
    ...
    grid = GridSearchCV(estimator=estimator, param_grid = params, cv=5, scoring='accuracy', n_jobs=-1)
...
def main():
        ...
        estimators=[
            ('logistic',LogisticRegression(solver='liblinear',penalty='l2'), {'C': [0.01, 0.1, 1, 10, 100]}),
            ('lasso',LogisticRegression(solver='liblinear',penalty='l1'), {'C': [0.001, 0.01, 0.1, 1, 10, 100, 1000]}),
            ('elasticnet',SGDClassifier(loss='log_loss', penalty='elasticnet'), 
                            {'alpha': [0.0001, 0.001, 0.01],
                            'l1_ratio': [0.15, 0.5, 0.85],
                            'max_iter': [1000, 2000, 3000],
                            'eta0': [0.01, 0.1, 1]}),
            ('random_forest',RandomForestClassifier(n_estimators=100, random_state=0), {'n_estimators': [10, 50, 100, 200], 'max_depth': [3, 5, 7, None]}),
        ...
        dataset, target, train_x, val_x, eval_x, train_y, val_y, eval_y = load_data(data, target)
